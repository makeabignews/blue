...
def init():
    ...
    send("请打开温度计",'192.168.2.74',56789)
    time.sleep(5)
def get_value():
    #process = os.popen("sudo gatttool -b C6:05:04:03:F1:BF --char-read -a 0x001b")
    process = os.popen("sudo gatttool -b C6:05:04:03:F1:B0 --char-read -a 0x001b")
    value = process.read()
    return value
def out_txt(str):
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if(state=="00"):
            ...
            send(text,'192.168.2.74',45678)
        if(state=="40" or state=="42"):
            post="unknowUser_%s" % temp_str
            print ("ok")
            #send(text,'192.168.2.22',33333)
            send(text,'192.168.2.74',56789)
